import React from 'react';

const PerfectTest = (): JSX.Element => {
  return (
    <section>
      <h4>
        1) Welche der folgenden Verben benutzt man mit dem Hilfsverb „haben“ und
        welche mit dem Hilfsverb „sein“? Bilden Sie zwei Gruppen:
      </h4>
      gehen, machen, haben, werden, schwimmen, lesen, sein, sich kämen,
      aufwachen, können
      <h4>
        Bilden Sie die Sätze aus den Wörtern. Achten Sie auf die richtige
        Reihenfolge:
      </h4>
      <ol>
        <li>Der Witz, super toll, hat, geklappt.</li>
        <li>Habt, den Film, ihr, schon, gesehen, gestern.</li>
        <li>Wir, dir, geglaubt, haben, umsonst.</li>
        <li>über deine Lüge, meine Freundin, gesagt, hat, mir.</li>
        <li>Erika, selbst, das Bild, in der Schule, gemacht, hat.</li>
      </ol>
      <h4>3) Bilden Sie das Partizip II der folgenden schwachen Verben:</h4>
      bremsen, malen, hören, basteln, bellen, zeigen, prüfen, kochen, legen,
      meinen
      <h4>Setzen Sie die Verben in Klammern im Perfekt ein:</h4>
      <ol>
        <li>Ich habe wieder alles …. . Nichts zu danken! (retten)</li>
        <li>Es hat …., danach geschneit. Was für Winter! (regnen)</li>
        <li>Wir haben schon mit ihr … (reden)</li>
        <li>Ich habe das Buch …., aber es war sinnlos. (öffnen)</li>
        <li>Du hast zu Laut …! Sie haben uns erwischt! (atmen)</li>
      </ol>
      <h4>
        5) Schreiben Sie die folgenden Sätze im Perfekt um. Benutzen Sie dabei
        Wörter in den Klammern.
      </h4>
      <ol>
        <li>Wir studieren zusammen Philosophie in Berlin (vor 5 Jahren)</li>
        <li>
          Eine peinliche Pause entsteht jedes Mal, wenn wir uns sehen (…, wenn
          wir uns im Korridor gesehen haben)
        </li>
        <li>Das Bild beeindruckt mich sehr (damals im Museum)</li>
        <li>Sie kommen um 20 Uhr an. (schon längst)</li>
        <li>Die Vorlesung findet im Raum 229 statt (am Donnerstag)</li>
      </ol>
      <h4>6) Korrigieren Sie die Fehler in Bildung der Partizipien II</h4>
      Die Wörter hast du gut gediktiert, aber wer hat sie aufschrieben? Deine
      Schüler haben gar nicht zuhört. Viele sind von ferne hinfahren. Das lohnt
      sich aber nicht. Ich habe nicht dafür 5 Jahre gestudiert! Okay, reden wir
      über was anderes… Hat dir Uwe nicht geerzählt? Er hat begeschlossen, nach
      Berlin zu ziehen. Seltsam!
      <h4>
        Übersetzen Sie ins Deutsche. Benutzen Sie dabei die Wörter aus den
        Klammern
      </h4>
      <ol>
        <li>Ты съел суп? – Да, вчера. (essen)</li>
        <li>Я разорвала договор на мелкие кусочки. (zerreißen)</li>
        <li>Профессор зачитал свой доклад вслух. (vorlesen)</li>
        <li>Мы сидели за столом молча, в пустом доме (sitzen)</li>
        <li>Лара и Дэн улетели в Париж, даже не сказав «пока». (fliegen)</li>
      </ol>
      <h4>Sezen Sie die eingeklammerten Verben im Perfekt ein</h4>
      <p>
        Der Tag … von Anfang an … (schieflaufen). Der Wecker … kaputt … und Nina
        … das nicht rechtzeitig … (merken). Sie … (verschlafen)! Schnell … sie
        ihre Lieblingsjeans und ein T-Shirt … (anziehen). Doch sie … keine
        Socken … (finden) und nur die Zeit … (verlieren). Auf der Straße … sie …
        (feststellen), dass ihr Bus schon … (wegfahren). Hektisch … sie Nico …
        und er … sie … (anrufen, abholen). Im Zentrum der Stadt … sie aber eine
        halbe Stunde im Stau … (stehen). Der Unterricht … um halb zehn …
        (anfangen), aber sie … erst um 10 Uhr … (kommen). An der Uni … sie …
        (sehen): ihr Handy … zu Hause … (bleiben). Danach … sie sich zum Termin
        in dem Dekanat … (sich verspäten) und sich mit einer Freundin (sich
        streiten). Sie … nicht mal Kaffee … (trinken). Nach dem Unterricht … sie
        … (denken), dass ein neues Café um die Ecke … (sich öffnen) und …
        (bestimmen), dorthin zu gehen. Dort … sie wunderbaren Kaffee (probieren)
        und ein leckeres Brötchen (essen). Das … ihren Tag … (retten).
      </p>
      <p>
        Letztendlich … alles bestens (klappen). Sie … ihren Dozenten mit ihrem
        Zeitmanagement (beeindrucken), einen Tag ohne Handy (verbringen) und
        noch Spaß (haben). Was für einen Tag!
      </p>
    </section>
  );
};

export default PerfectTest;
